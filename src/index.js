...
function padHandler(event) {
 ...
 if (!color) return;
 const pad = pads.find(x => x.color === color);
 pad.sound.play();
 checkPress(color);
 return color;
}
...
async function activatePad(color) {
   const pad = pads.find(x => x.color === color);
   const padElement = pad.selector
...
function playHumanTurn() {
   ...
   setText(statusSpan, `Your turn player: ${computerSequence.length} presses left`);
   setText(heading, `Round ${roundCount} of ${maxRoundCount}`);
}
